        ...
        public async Task<ObjectResult> PROCESSAR_EMAILS()
        {
            ...
            lock (_lock)
            {
                if (_instanciaAtiva)
                {
                    ...
                    string Mensagem = "Uma instância já está em execução. Tente novamente mais tarde.";
            ...
            catch(Exception ex)
            {
                ...
                ExceptionModel exceptionEstruturada = new ExceptionModel(ex, true);

                // Registra o Log
                EmarADO.GERA_LOG(
                _configuration.GetValue<string>("ConnectionStrings:DefaultConnection"),
                JsonConvert.SerializeObject(exceptionEstruturada)
                );

                // Retorna a consulta
                return APIResponseHelper.EstruturaResponse(
                    "Ops",
                    exceptionEstruturada.Mensagem,
                    "error",
                    exceptionEstruturada,
                    400,
                    Url.Action("processar_emails", "Emar", null, Request.Scheme));
            }
            finally
